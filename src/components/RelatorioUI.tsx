...
export default function RelatorioUI({ tasks, userName, onTriggerToast, defaultOpen }: RelatorioUIProps) {
  ...
  const handleExportPDF = () => {
    ...
    if (onTriggerToast) {
      onTriggerToast('Gerando relatório em PDF...');
    }
    ...
    doc.save(`vall-relatorio-${safeStartStr}-a-${safeEndStr}.pdf`);

    if (onTriggerToast) {
      onTriggerToast('Download concluído com sucesso!');
    }
  };
    ...
    <div className="space-y-4 relative z-10 text-left" id="component_relatorio_ui">
      ...
      <div className="pt-2">
        <button
          ...
          className="w-full text-xs font-bold bg-[#2DD4BF] hover:bg-[#5eead4] text-black py-3 px-5 rounded-2xl transition cursor-pointer active:scale-95 min-h-[48px] flex items-center justify-center gap-2 font-mono shadow-[0_0_20px_rgba(45,212,191,0.25)]"
